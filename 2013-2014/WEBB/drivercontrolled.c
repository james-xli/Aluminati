...
void single_main_wheel(int joy_x, int joy_y) // because patrick
{
	 float k = .35;
	 int drivePower, turnPower;

   drivePower = - SINGLE_DEADBAND_WHEEL(k * joy_y); //negative for chain drive
   turnPower  = - SINGLE_DEADBAND_WHEEL(joy_x); //negative for chain drive
...
void servo_out (int btn_x)
{
		...
		int out2; //spit blocks out for servo2 (right?? EDIT)
		out = servo_go;
		out2 = servo_go1;
		if (btn_x == 1)
	{
...
Servo value of 256 = Full Power/Speed Forward*/
int servo_swallow = 0;
int servo_swallow1 = 250;
void servo_in (int btn_x)
{
		int in; //spit blocks out for servo1 (left?? EDIT)
		int in2; //spit blocks out for servo2 (right?? EDIT)
		in = servo_swallow;
		in2 = servo_swallow1;
		if (btn_x == 1)
	{
...
{
		int no; //stops servo from rotating
		no = servo_no;
		if (btn_x == 1)
	{
		servo[servo1] = no;
