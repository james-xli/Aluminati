...
#define CLAWSERVOHIGH(x) (((x) > 255 )? 255: (x))
#define CLAWSERVOLOW(x)	(((x) < 152)? 152: (x))
#define ARMSERVOHIGH(x) (((x) > 200)? 200: (x))
#define ARMSERVOLOW(x)	(((x) < 0)? 0: (x))
...
void arm_servos ( int btn2, int btn4 )
{
	...
	if ( btn2 == 1 )
		arm_servo_angle = ARMSERVOHIGH(arm_servo_angle + arm_step_angle);

	else if ( btn4 == 1 )
		arm_servo_angle = ARMSERVOLOW(arm_servo_angle - arm_step_angle);

	servo[armServo] = arm_servo_angle; //locked between 0 and 255
...
void claw_servo ( int hat )
{
	...
	if ( hat == 0 )
	{
		claw_servo_angle = CLAWSERVOHIGH(claw_servo_angle + claw_step_angle);
	}
	else if ( hat == 4 )
	{
		claw_servo_angle = CLAWSERVOLOW(claw_servo_angle - claw_step_angle);
	}
