...
#pragma config(Sensor, S4,     IRSeekB,        sensorNone)
...
#define NOOB_BOUND_WHEEL(x) (((x) > 100)? 100: ((x) < -100)? -100: (x)) //*!!Makes sure that the joystick is between -100 and 100 !!*//
#define NOOB_DEADBAND_WHEEL(x) ((abs(x) >= 10)? x: 0)

void move_Base( int joy_x, int joy_y )
{
	 float k=1;
	 int drivePower, turnPower;

   joy_y = k*joy_y;
   turnPower = NOOB_DEADBAND_WHEEL(joy_x);
   drivePower  = NOOB_DEADBAND_WHEEL(joy_y);

   motor[RightBase] = NOOB_BOUND_WHEEL(drivePower + turnPower);
   motor[LeftBase] = NOOB_BOUND_WHEEL(drivePower - turnPower);
}
...
*/
//-----  ARM LIFT ----
#define BOUND_ARM(x) (((x) > 70)? 70: ((x) < -50)? -15: (x)) //Makes sure that the joystick is between -100 and 100 (limits big)
#define DEADBAND_ARM(x) ((abs(x) >= 4)? x: 0) //limits sensitivity (small)

void lift(int joy_k)
{
	float k = 1 ;
	int drivePower = DEADBAND_ARM(joy_k);
	drivePower = drivePower/k;
...
void servo_horizontal(int btn_z){
	if (btn_z == 1)
	{
		servo[servo1] = 120;
	}
...
task main()
{
	...
	while(true)
	{
		...
		if(joy1Btn(5) == 1 ){
			...
			move_Base(joystick.joy1_x2, joystick.joy1_y2);
		}
		...
		if ( joy2Btn(7)== 1 )
			...
			resetElevEncoder(joy2Btn(8));
			displayMessage(joy2Btn(1));

		servoInitial(joy2Btn(3));
