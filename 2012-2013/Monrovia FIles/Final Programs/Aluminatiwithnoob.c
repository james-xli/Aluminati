...
void noob_main_wheel( int joy_x, int joy_y )
{
	 float k=1;
	 int drivePower, turnPower;

   joy_y = k*joy_y;
   drivePower = NOOB_DEADBAND_WHEEL(joy_x);
   turnPower  = NOOB_DEADBAND_WHEEL(joy_y);
...
task main()
{
	...
	while(true)
	{
		...
		if (joy1Btn(6) == 1)
			noob_main_wheel(joystick.joy1_x2, joystick.joy1_y2);
		else{
