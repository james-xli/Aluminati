...
#define SERVO_BOUND(X) ((X > 255)? 255 : ((X<0)? 0: X))
void servo_hat(int hat){
	int servo_val = ServoValue[servo1];
	if(hat == 4){
		servo[servo1] = servo_val + 15;
	}
	else if(hat == 0){
		servo[servo1] = servo_val - 15;
	}
...
task main()
{
	...
	while(true)
	{
		...
		servoDrop(joy2Btn(2));
		servo_hat(joystick.joy2_TopHat);
